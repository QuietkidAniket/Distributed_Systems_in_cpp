 ...
 * @date 4th April, 2026
 * 
 * @brief In the multi-threaded Reactor Pattern, we split the jobs:
 * The Main Thread (The Host): Runs the poll() loop. Its only job is to answer the door, 
 * take the order (read the network string), and hand the ticket to the kitchen.
 * The Thread Pool (The Chefs): Grabs the ticket, safely interacts with the shared database,
 *  cooks the result, writes it back to the client, and goes back to sleep.
 * The Database (The Pantry): Your std::unordered_map, protected by a std::shared_mutex 
 * so multiple chefs don't collide.
 * 
*/
...
class Redis{
  ...
  ThreadSafeCache<std::string, std::string> cache;
  ...
  std::string delete_(const std::string& key){
    cache.remove(key);
    return "Deleted\n";
  }
  public:
  ...
  int run(){
    ...
    // create the thread pool of the maximum number of threads supported by the system
      ...
      for(size_t i = 0; i < fds.size(); i++){
        if(fds[i].revents & POLLIN){
          if(fds[i].fd == server_fd){ // incoming new client connection via the server socket
            ...
            if( bytes_read <= 0){ // Termination of the connection
              close(sender_fd);
              fds[i].fd=  -1;
              cleanup_needed = true;
